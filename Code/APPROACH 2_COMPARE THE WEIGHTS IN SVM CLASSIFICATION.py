...
=> chanel 4, chanel 12, chanel 19: all absolute value of weights in H chanel is higher than in P chanel, except for 1 subject.
...
ax.axhline(y, linestyle ='--',antialiased = True)
...
#chanel 12
fig, ax = plt.subplots() #can use format plt.figure(figsize=(10, 12))
H_subjects_chanel_12 = df[12][0:6]
P_subjects_chanel_12 = df[12][6:]

box = ax.boxplot([H_subjects_chanel_12, P_subjects_chanel_12], patch_artist=True,vert=1, 
                  labels=['Healthy', 'PTSD'], widths = 0.4)
colors = ['cyan', 'lightblue', ]
for patch, color in zip(box['boxes'], colors):
   patch.set_facecolor(color)
#http://www.datasciencemadesimple.com/box-plot-in-python/
y=min(H_subjects_chanel_12)
...
#chanel 19
fig, ax = plt.subplots() #can use format plt.figure(figsize=(10, 12))
H_subjects_chanel_19 = df[19][0:6]
P_subjects_chanel_19 = df[19][6:]

box = ax.boxplot([H_subjects_chanel_19, P_subjects_chanel_19], patch_artist=True,vert=1, 
                  labels=['Healthy', 'PTSD'], widths = 0.4)
colors = ['cyan', 'lightblue', ]
for patch, color in zip(box['boxes'], colors):
   patch.set_facecolor(color)
#http://www.datasciencemadesimple.com/box-plot-in-python/
y=min(H_subjects_chanel_19)
plt.axhline(y, linestyle ='--',antialiased = True)
trans = transforms.blended_transform_factory(ax.get_yticklabels()[0].get_transform(), ax.transData)
ax.text(0,y, "{:.6f}".format(y), color="red", transform=trans, ha="right", va="center")
#https://stackoverflow.com/questions/42877747/add-a-label-to-y-axis-to-show-the-value-of-y-for-a-horizontal-line-in-matplotlib
